...
from  dbo._Failures2015_1 f with(nolock)
left outer join dbo._attachments2015_1 a with(nolock) on f.F_ID=a.f_id
...
where exists (select 1 from TestRecords tr with(nolock) where tr.TestUnitID=tu.ID and tr.TestStageID=ts.ID and tr.TestID=t.id)
...
select distinct 
	...
	1 as RelabVersion,
	'REMI' as LastUser,
	(select top 1 f2.FAILURE_COMMENT from _Failures2015_1 f2 where f2.UNIT_NUMBER=f.UNIT_NUMBER and f2.DROP_NUM=f.DROP_NUM and f2.JU_ID=f.JU_ID) as Comment,
	3 as ResultSource,
	null as FailDocRQID,
	t.ID as TestID,
...
FROM dbo._Failures2015_1 f with(nolock)
left outer join dbo._attachments2015_1 a on f.F_ID=a.f_id
left outer join dbo._FailureModes fm with(nolock) on fm.FM_ID=f.FM_ID
left outer join dbo._FailureModes fmp with(nolock) on fmp.FM_ID=fm.PARENT_FM_ID
left outer join dbo._FailureModes fmp2 with(nolock) on fmp2.FM_ID=fmp.PARENT_FM_ID
left outer join dbo._FailureModes fmp3 with(nolock) on fmp3.FM_ID=fmp2.PARENT_FM_ID
...
inner join TestStages ts with(nolock) on ts.JobID=j.ID and ts.TestStageName like convert(nvarchar,f.DROP_NUM) + ' %' and ts.TestStageName like '%' + convert(nvarchar,f.[TYPE]) + '%'
inner join Tests t with(nolock) on ts.TestID=t.id
where  not exists (select 1 from TestRecords tr with(nolock) where tr.TestUnitID=tu.ID and tr.TestStageID=ts.ID and tr.TestID=t.id)
...
print '2'
 ...
 select distinct 
	...
	1 as RelabVersion,
	'REMI' as LastUser,
	(select top 1 f2.FAILURE_COMMENT from _Failures2015_1 f2 with(nolock) where f2.UNIT_NUMBER=f.UNIT_NUMBER and f2.DROP_NUM=f.DROP_NUM and f2.JU_ID=f.JU_ID) as Comment,
	3 as ResultSource,
	null as FailDocRQID,
...
IF NOT EXISTS (select r.*
	...
	inner join TestStages ts with(nolock) on ts.JobID=j.ID and ts.TestStageName like convert(nvarchar,f.DROP_NUM) + ' %' and ts.TestStageName like '%' + convert(nvarchar,f.[TYPE]) + '%'
	inner join Tests t with(nolock) on ts.TestID=t.id
	where  r.TestID=t.ID and r.TestStageID=ts.ID and r.TestUnitID=tu.ID))
	BEGIN
	...
	inner join Jobs j with(nolock) on j.JobName=b.JobName
	inner join TestStages ts with(nolock) on ts.JobID=j.ID and ts.TestStageName='baseline'
	where  r.TestID=ISNULL((select top 1 testid from testrecords r where r.testunitid=tu.id and r.teststagename like '%post%'),1280) and r.TestStageID=ts.ID and r.TestUnitID=tu.ID))
	BEGIN
		insert into Relab.Results(TestStageID,TestID,TestUnitID,PassFail)
...
print '6'
	...
	inner join Jobs j with(nolock) on j.JobName=b.JobName
	inner join TestStages ts with(nolock) on ts.JobID=j.ID and ts.TestStageName = 'baseline'
	where r.TestID=isnull((select top 1 testid from testrecords r where r.testunitid=tu.id and r.teststagename like '%post%'),1280)
