CREATE PROCEDURE Relab.remispResultsGraph @MeasurementTypeID INT, @batchIDs NVARCHAR(MAX), @UnitIDs NVARCHAR(MAX), @TestID INT, @ParameterName NVARCHAR(255)=NULL, @ParameterValue NVARCHAR(250)=NULL, @ShowUpperLowerLimits INT = 1, @Xaxis INT, @PlotValue IN
T, @IncludeArchived INT = 0, @Stages NVARCHAR(MAX)  
AS  
BEGIN  
 ...
 CREATE TABLE #Graph (YAxis NVARCHAR(500), XAxis NVARCHAR(500), LoopValue NVARCHAR(500), LowerLimit NVARCHAR(255), UpperLimit NVARCHAR(255), QRANumber NVARCHAR(11), paramName NVARCHAR(500), paramValue NVARCHAR(500), BatchUnitNumber INT, ReTestNum INT, par
amValueNumeric INT)  
 DECLARE @COUNT INT  
 ...
 */  
 ...
 BEGIN  
  ...
  WHEN ('''+ ISNULL(@ParameterValue,'')+''' = '''' And '''+ ISNULL(@ParameterName,'')+''' <> '''') THEN Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' = '''' THEN ''V'' ELSE ''N'' END) + '': '' + Relab.ResultsParamet
ersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN ''V'' ELSE ''N'' END)  
  ELSE ISNULL(Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN ''N'' ELSE ''V'' END), '''') + '': '' + ISNULL(Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' = '''' TH
EN ''V'' ELSE ''N'' END), '''') END AS XAxis,   
 ...
 BEGIN  
  ...
  CASE WHEN '''+ ISNULL(@ParameterName,'')+''' <> '''' AND '''+ ISNULL(@ParameterValue,'')+''' = '''' THEN  
   ISNULL(Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' = '''' THEN ''N'' ELSE ''V'' END), '''') + '': '' + ISNULL(Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' = '''' THEN ''
V'' ELSE ''N'' END), '''')  
  ELSE ''''   
  END +   
 ...
 BEGIN  
  SET @query = 'INSERT INTO #Graph SELECT rm.MeasurementValue AS YAxis,  
  SUBSTRING(j.JobName, 0, CHARINDEX('' '', j.Jobname, 0)) + '' '' + ts.TestStageName + '':'' + CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN ''
N'' ELSE ''V'' END) + '': '' + Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN ''V'' ELSE ''N'' END)   
  ELSE Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' = '''' THEN ''N'' ELSE ''V'' END) + '': '' + Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' = '''' THEN ''V'' ELSE ''N'' EN
D) END AS XAxis,  
  SUBSTRING(j.JobName, 0, CHARINDEX('' '', j.Jobname, 0)) + '' '' + ts.TestStageName + '' '' + ' + CASE WHEN @IncludeArchived = 1 THEN ' CONVERT(VARCHAR, tu.BatchUnitNumber) + '' '' + convert(varchar,rm.ReTestNum) ' ELSE ' CONVERT(varchar, tu.BatchUnitNum
ber) ' END + ' AS LoopValue,    
  rm.LowerLimit, rm.UpperLimit, Batches.QRANumber, ISNULL(Relab.ResultsParametersNameComma(rm.ID, ''N''), '''') AS ParamName, ISNULL(Relab.ResultsParametersNameComma(rm.ID, ''V''), '''') AS ParamValue,  
  tu.BatchUnitNumber, rm.ReTestNum, ISNUMERIC(ISNULL(Relab.ResultsParametersNameComma(rm.ID, ''V''), '''')) AS paramValueNumeric '  
 END  
 ...
 BEGIN  
  SET @query = 'INSERT INTO #Graph SELECT rm.MeasurementValue AS YAxis,   
  SUBSTRING(j.JobName, 0, CHARINDEX('' '', j.Jobname, 0)) + '':'' + CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN ''N'' ELSE ''V'' END) + '': '
' + Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN ''V'' ELSE ''N'' END)   
  ELSE Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' <> '''' THEN ''V'' ELSE ''N'' END) +'': '' + Relab.ResultsParametersNameComma(rm.ID, CASE WHEN '''+ ISNULL(@ParameterValue,'')+''' = '''' THEN ''V'' ELSE ''N'' EN
D) END AS XAxis,   
  ' + CASE WHEN @IncludeArchived = 1 THEN ' SUBSTRING(j.JobName, 0, CHARINDEX('' '', j.Jobname, 0)) + '' '' + ts.TestStageName + '' '' + CONVERT(VARCHAR, tu.BatchUnitNumber) + '' '' + convert(varchar,rm.ReTestNum) ' ELSE ' SUBSTRING(j.JobName, 0, CHARINDE
X('' '', j.Jobname, 0)) + '' '' + ts.TestStageName + '' '' + CONVERT(VARCHAR, tu.BatchUnitNumber) ' END + ' AS LoopValue,   
  rm.LowerLimit, rm.UpperLimit, Batches.QRANumber, ISNULL(Relab.ResultsParametersNameComma(rm.ID, ''N''), '''') AS ParamName, ISNULL(Relab.ResultsParametersNameComma(rm.ID, ''V''), '''') AS ParamValue,  
  tu.BatchUnitNumber, rm.ReTestNum, ISNUMERIC(ISNULL(Relab.ResultsParametersNameComma(rm.ID, ''V''), '''')) AS paramValueNumeric '  
 END  
